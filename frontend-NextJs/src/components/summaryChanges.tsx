...
const SummaryChanges: React.FC<{ summaryChangesData: SummaryChangesData }> = ({ summaryChangesData }) => {
    ...
    <div>
        ...
        <Typography variant="body1" sx={{ color: 'grey.800', marginBottom: 0, lineHeight: '1.8' }}>
            {Object.keys(column_changes).length > 0 ? <span>Step 3: We identified all columns we changed below, with a count of values or cells modified.</span> : null}
        </Typography>
        ...
        <Typography variant="body1" sx={{ color: 'grey.800', marginTop: 1, lineHeight: '1.8' }}>
            {removed_columns.length > 0 ? <span>Lastly, we also <b>{removed_columns.length}</b> removed columns that did not contain any data: <b>{removed_columns.join(', ')}</b></span> : null}
        </Typography>
    </div>
