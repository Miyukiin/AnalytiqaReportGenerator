...
const SummaryChanges: React.FC<{ summaryChangesData: SummaryChangesData }> = ({ summaryChangesData }) => {
    ...
    <div>
        <Typography variant="body1" sx={{ color: 'grey.800', marginBottom: 0, lineHeight: '1.8' }}>
            {rows_removed && rows_removed!== null ? <span>Step 1: We removed <b>{rows_removed}</b> duplicate rows.</span> : null}
        </Typography>
        <Typography variant="body1" sx={{ color: 'grey.800', marginBottom: 0, lineHeight: '1.8' }}>
            {missing_values_replaced && missing_values_replaced !== null ? <span>Step 2: We replaced <b>{missing_values_replaced}</b> missing values.</span> : null}
        </Typography>
        <Typography variant="body1" sx={{ color: 'grey.800', marginBottom: 0, lineHeight: '1.8' }}>
            {column_changes && Object.keys(column_changes).length > 0 ? <span>Step 3: We identified all columns we changed below, with a count of values or cells modified.</span> : null}
        </Typography>
